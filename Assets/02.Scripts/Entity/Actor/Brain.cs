...
public class Brain
{
    ...
    public async UniTask Act(ActParameterResult paramResult, CancellationToken token)
    {
        ...
        await ExecuteAction(new AgentAction
        {
    ...
    public async UniTask ThinkAndAct()
    {
        ...
        while (retryCount < maxRetries)
        {
            ...
            await Act(paramResult, CancellationToken.None); // Pass CancellationToken.None for now
    ...
    private async UniTask ExecuteAction(AgentAction action, CancellationToken token)
    {
        // CancellationToken은 내부에서 사용하지 않음 (actionExecutor가 지원하지 않음)
        var result = await actionExecutor.ExecuteActionAsync(new ActionReasoning
        {
    ...
    // 이벤트 기반 Think/Act 루프
    public async UniTask ThinkAndActAsync()
    {
        thinkActCts?.Cancel(); // 기존 루프 취소
        ...
        try
        {
            ...
            // 2. Act
            await Act(paramResult, token);
            token.ThrowIfCancellationRequested();
            // 3. Act가 끝나면 다시 Think (이벤트가 없으면)
            await ThinkAndActAsync();
        }
