...
{    
    ...
    private async UniTask<string> ProcessInventoryBoxInteraction(Actor actor, CancellationToken cancellationToken)
    {
        try
        {
            ...
            string reasoning = $"협탁과 상호작용하여 아이템을 관리합니다. 현재 아이템 수: {items.Count}, 최대: {maxItems}";
            string intention = "협탁에 아이템을 추가하거나 협탁에서 아이템을 가져옵니다.";
            
            if (actResult != null)
            {
                reasoning = actResult.Reasoning;
                intention = actResult.Intention;
                Debug.Log($"[BedsideTable] ActorManager에서 가져온 원본 값 - Reasoning: {reasoning}, Intention: {intention}");
            }
            else
            {
                Debug.LogWarning($"[BedsideTable] ActorManager에서 {actor.Name}의 ActSelectResult를 찾을 수 없습니다. 기본값 사용");
            }
        ...
        {
            Debug.LogWarning($"[BedsideTable] InventoryBoxAgent 실행 실패: {ex.Message}");
    ...
    private string ExecuteAddAction(Actor actor, List<string> itemNames)
    {
        ...
        {
            result += $"{actor.Name}이(가) {string.Join(", ", addedItems)}을(를) 협탁에 올렸습니다. ";
        }
    ...
    private string ExecuteRemoveAction(Actor actor, List<string> itemNames)
    {
        ...
        {
            result += $"{actor.Name}이(가) 협탁에서 {string.Join(", ", removedItems)}을(를) 가져왔습니다. ";
        }
