...
public class DiningTable : InventoryBox
{
    ...
    private async UniTask<string> ProcessInventoryBoxInteraction(Actor actor, CancellationToken cancellationToken)
    {
        try
        {
            ...
            string reasoning = $"식탁과 상호작용하여 아이템을 관리합니다. 현재 아이템 수: {items.Count}, 최대: {maxItems}";
            string intention = "식탁에 아이템을 추가하거나 식탁에서 아이템을 가져옵니다.";
            
            if (actResult != null)
            {
                reasoning = actResult.Reasoning;
                intention = actResult.Intention;
                Debug.Log($"[DiningTable] ActorManager에서 가져온 원본 값 - Reasoning: {reasoning}, Intention: {intention}");
            }
            else
            {
                Debug.LogWarning($"[DiningTable] ActorManager에서 {actor.Name}의 ActSelectResult를 찾을 수 없습니다. 기본값 사용");
            }
        ...
        {
            Debug.LogWarning($"[DiningTable] InventoryBoxAgent 실행 실패: {ex.Message}");
    ...
    private string ExecuteAddAction(Actor actor, List<string> itemNames)
    {
        ...
        {
            result += $"{actor.Name}이(가) {string.Join(", ", addedItems)}을(를) 식탁에 올렸습니다. ";
        }
    ...
    private string ExecuteRemoveAction(Actor actor, List<string> itemNames)
    {
        ...
        {
            result += $"{actor.Name}이(가) 식탁에서 {string.Join(", ", removedItems)}을(를) 가져왔습니다. ";
        }
    ...
    private string ExecuteFallbackInteraction(Actor actor)
    {
        ...
        return $"식탁에 아이템을 놓을 수 있습니다. 현재 {items.Count}개, 최대 {itemPlacementPositions.Count}개";
    }
