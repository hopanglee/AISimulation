...
public class Locker : InventoryBox
{
    ...
    private async UniTask<string> ProcessInventoryBoxInteraction(Actor actor, CancellationToken cancellationToken)
    {
        try
        {
            ...
            string reasoning = $"사물함 {lockerNumber}과 상호작용하여 아이템을 관리합니다. 현재 아이템 수: {items.Count}, 최대: {maxItems}";
            string intention = "사물함에 아이템을 추가하거나 사물함에서 아이템을 가져옵니다.";
            
            if (actResult != null)
            {
                reasoning = actResult.Reasoning;
                intention = actResult.Intention;
                Debug.Log($"[Locker] ActorManager에서 가져온 원본 값 - Reasoning: {reasoning}, Intention: {intention}");
            }
            else
            {
                Debug.LogWarning($"[Locker] ActorManager에서 {actor.Name}의 ActSelectResult를 찾을 수 없습니다. 기본값 사용");
            }
        ...
        {
            Debug.LogWarning($"[Locker] InventoryBoxAgent 실행 실패: {ex.Message}");
    ...
    private string ExecuteAddAction(Actor actor, List<string> itemNames)
    {
        ...
        {
            result += $"{actor.Name}이(가) {string.Join(", ", addedItems)}을(를) 사물함 {lockerNumber}에 넣었습니다. ";
        }
    ...
    private string ExecuteRemoveAction(Actor actor, List<string> itemNames)
    {
        ...
        {
            result += $"{actor.Name}이(가) 사물함 {lockerNumber}에서 {string.Join(", ", removedItems)}을(를) 가져왔습니다. ";
        }
