...
public class SpecificPlannerAgent : GPT
{
    ...
    private List<string> GetAvailableLocations()
    {
        try
        {
            ...
            Debug.Log($"[SpecificPlannerAgent] 전체 Area 수: {allAreas.Count}");
            ...
            foreach (var area in areas)
            {
                ...
                Debug.Log($"[SpecificPlannerAgent] Area 추가: {area.locationName} -> 전체 경로: {fullLocationPath}");
            }

            Debug.Log($"[SpecificPlannerAgent] 최종 사용 가능한 장소 목록 ({locations.Count}개): {string.Join(", ", locations)}");

            if (locations.Count == 0)
            {
                Debug.LogError("[SpecificPlannerAgent] 사용 가능한 장소가 없습니다! 기본 장소를 사용합니다.");
                return null;
            }
        ...
        {
            Debug.LogError($"[SpecificPlannerAgent] 장소 목록 가져오기 실패: {ex.Message}");
            throw new System.InvalidOperationException($"SpecificPlannerAgent 장소 목록 가져오기 실패: {ex.Message}");
        }
    }
